...
def train(model, data, train_idx, optimizer, model_type):
    model.train()
    optimizer.zero_grad()
...
@torch.no_grad()
@torch.no_grad()
def test(model, data, train_idx, valid_idx, test_idx, model_type, metric='accuracy'):
    model.eval()
...
def main(args):
    ...
    for run in range(args.runs):
        ...
        if args.dataset in ['texas','cornell','wisconsin','actor', 'tolokers','roman-empire','amazon-ratings','questions','minesweeper']:
        ...
        if args.model_type == 'GCN':
            model = GCN(data.num_features, args.hidden_channels, out_dim,
                        args.num_layers, args.dropout)
        elif args.model_type == 'SAGE':
            model = SAGE(data.num_features, args.hidden_channels, out_dim,
                         args.num_layers, args.dropout)
        elif args.model_type == 'GAT':
            model = GAT(data.num_features, args.hidden_channels, out_dim,
                        args.num_layers, args.dropout)
        elif args.model_type == 'LINKX':
            model = LINKX(len(data.y), data.num_features, args.hidden_channels,
                          out_dim, args.num_layers, 1, 1, args.dropout)
        elif args.model_type == 'MLP':
            model = MLP(data.num_features, args.hidden_channels, out_dim,
                        args.num_layers, args.dropout)
        else:
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            loss = train(model, data, train_idx, optimizer, args.model_type)
            ...
            if args.dataset in ['questions', 'minesweeper','tolokers']:
                result = test(model, data, train_idx, valid_idx, test_idx,
                              args.model_type, metric='roc_auc')
            else:
                result = test(model, data, train_idx, valid_idx, test_idx,
                              args.model_type, metric='accuracy')
...
if __name__ == "__main__":
    ...
    datasets_to_run =  ['cora','citeseer','pubmed','physics','cs','texas','cornell','wisconsin','chameleon', 'squirrel','actor','amazon-ratings','questions','tolokers']
