...
def main(args):
    ...
    #################### load the features
    CC_domain=torch.load(f'CC_{args.dataset}.pt').to(device)
    ...
    data.x = CC_domain  # Replace original features with wise embeddings    print(data)
    print("data.x device:", data.x.device)
    data=data.to(device)
    print("CC_domain device:", CC_domain.device)
    print("data.x device:", data.x.device)
...
if __name__ == "__main__":
    datasets_to_run = ['cora','citeseer','pubmed','wikics', 'computers','photo','physics','cs','roman-empire','amazon-ratings','actor','questions','minesweeper']  # Add more as needed
